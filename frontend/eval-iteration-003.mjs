...
async function testInventoryPage() {
  ...
  try {
    ...
    await page.goto('http://localhost:3000/inventory', { waitUntil: 'domcontentloaded', timeout: 30000 });
    
    // Wait for either table or login form
    const loginForm = await page.locator('form, [class*="login"]').first().isVisible().catch(() => false);
    if (loginForm) {
      console.log('   ⚠ Login page detected, trying demo credentials...');
      // Try to fill login form
      await page.fill('input[type="text"]', 'admin@example.com').catch(() => null);
    ...
    for (const toggle of themeToggles) {
      ...
      const title = await toggle.getAttribute('title').catch(() => '');
    ...
    if (!foundToggle) {
      console.log(`   ⚠ Theme toggle not found, checking for manual dark mode`);
    }
    ...
    }).catch(() => ({color: 'unknown', bg: 'unknown'}));
