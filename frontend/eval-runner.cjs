...
const screenshotDir = path.join(__dirname, '..', 'gan-harness', 'screenshots');
...
async function evaluateInventory() {
  ...
  try {
    ...
    const designQualityData = await page.evaluate(() => {
      ...
      const textElements = Array.from(document.querySelectorAll('[class*="text-"]'));
      textElements.forEach(el => {
        const cls = el.getAttribute('class');
        if (typeof cls === 'string') {
          cls.split(/\s+/).forEach(c => {
            if (c.match(/^text-(xs|sm|base|lg|xl|2xl|3xl|4xl)/)) {
              textSizes.add(c);
            }
          });
        }
      ...
      const spacingElements = Array.from(document.querySelectorAll('[class*="p-"], [class*="gap-"], [class*="m-"], [class*="space-"]'));
      spacingElements.forEach(el => {
        const cls = el.getAttribute('class');
        if (typeof cls === 'string') {
          cls.split(/\s+/).forEach(c => {
            if (c.match(/^(p|gap|m|space)-(0\.5|1|1\.5|2|2\.5|3|4|6|8|12)/)) {
              spacing.add(c);
            }
          });
        }
    ...
    const craftData = await page.evaluate(() => {
      ...
      data.reduced_motion_honored = !prefersReduced;
    ...
    const functionalityData = await page.evaluate(() => {
      ...
      const densityControls = document.querySelectorAll('[data-testid*="density"], [class*="Density"]');
      data.has_density_toggle = densityControls.length > 0;
...
evaluateInventory().then(results => {
  fs.writeFileSync(path.join(__dirname, '..', 'evaluation_results.json'), JSON.stringify(results, null, 2));
