...
ARRAY_BODY = {"name": "syn-01", "vendor": "synology"}
...
def test_viewer_cannot_create(client, db_session):
    ...
    r = client.post("/api/storage/arrays", headers=auth_headers(csrf), json=ARRAY_BODY)
...
def test_editor_create_without_csrf_forbidden(client, db_session):
    ...
    login(client, "e@x.io")
    r = client.post("/api/storage/arrays", json=ARRAY_BODY)
    assert r.status_code == 403


def test_unauthenticated_list_forbidden(client, db_session):
    r = client.get("/api/storage/arrays")
