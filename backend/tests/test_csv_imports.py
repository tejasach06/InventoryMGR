...
def test_csv_preview_persists_classification_for_create_update_conflict_and_invalid(
    ...
    create_vm_row(db_session, editor, name="Existing App", external_id=None, environment="lab")
    csrf = login(client, "editor@example.local")

    csv_content = "\n".join(
        [
            "name,platform,environment,cluster,host,status,cpu_cores,memory_mb,disk_gb,external_id,ip_addresses,tags,ha_enabled,last_verified_at",
            "Existing App,Proxmox,lab,pve-cluster-a,pve01,running,4,8192,120,,"
            "10.0.0.10;10.0.0.11,web; critical,yes,2026-06-13",
            " existing app ,pve,lab,pve-cluster-a,pve01,stopped,2,4096,80,,,,false,",
            "New VMware,vcenter,prod,vc-cluster,esx01,unknown,8,16384,200,"
            "vm-200,192.168.1.20,db;prod,no,2026-01-01",
            "Broken Row,vmware,prod,vc-cluster,,unknown,-1,1024,20,,,,false,",
        ]
    )
    ...
    assert {error["field"] for error in rows[3]["errors"]} >= {"host", "cpu_cores"}
...
def test_csv_commit_refuses_invalid_or_conflicting_batches(client, db_session: Session) -> None:
    ...
    csv_content = "\n".join(
        [
            "name,platform,environment,cluster,host",
            "Existing App,proxmox,lab,pve-cluster-a,pve01",
            "existing app,proxmox,lab,pve-cluster-a,pve01",
        ]
    )
...
def test_csv_commit_uses_persisted_rows_and_rolls_back_when_later_upsert_fails(
    ...
    existing = create_vm_row(
        db_session, editor, name="Manual Proxmox", external_id=None, environment="lab"
    )
    csrf = login(client, "editor@example.local")
    csv_content = "\n".join(
        [
            "name,platform,environment,cluster,host,status,cpu_cores,memory_mb,disk_gb,criticality,lifecycle",
            "New VMware,vmware,prod,vc-cluster,esx01,running,2,4096,60,medium,active",
            "Manual Proxmox,proxmox,lab,pve-cluster-a,pve01,stopped,6,12288,150,critical,active",
        ]
    )
    preview = upload_csv(client, csrf, csv_content)
    assert preview.status_code == 201, preview.text
...
def test_import_rbac_and_batch_visibility(client, db_session: Session) -> None:
    ...
    csv_content = "\n".join(
        [
            "name,platform,environment,cluster,host",
            "Visible Batch VM,proxmox,lab,pve-cluster-a,pve01",
        ]
    )
